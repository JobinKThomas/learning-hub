 ...
 * durations across the curriculum hierarchy (Topic -> Section -> Module -> LearningPath).
 */
...
export const calculateTopicDurationFromContent = ({
  ...
  // 4. Interactive items (Notes, Playgrounds, Quizzes)
  ...
  // Floor of at least 5 mins (or 3 mins if very brief), round to integer
...
/**
 * Calculates Section duration from its child Topics or fallback content
...
export const calculateSectionDuration = (topics = [], sectionData = {}) => {
  ...
  // Fallback if no topics exist yet: calculate from section description and items
...
/**
 * Calculates Module duration from its child Sections (or topics)
...
/**
 * Calculates Learning Path estimated hours from its child Modules
 * @param {Array<Object>} modules
 * @param {Object} [pathData]
 * @returns {number} estimatedHours
 */
