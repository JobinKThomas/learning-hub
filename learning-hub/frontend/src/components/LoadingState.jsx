...
export default function LoadingState({
  ...
  if (variant === 'spinner') {
    return (
      ...
      >
        <div className="animate-spin rounded-full h-10 w-10 border-3 border-slate-200 border-t-indigo-600" />
    ...
    return (
      <div className={`space-y-4 ${className}`}>
        {Array.from({ length: count }).map((_, idx) => (
          <div
            key={idx}
            className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs animate-pulse flex items-center justify-between gap-4"
          >
      ...
      <div className={`max-w-5xl mx-auto space-y-6 ${className}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {Array.from({ length: count }).map((_, idx) => (
              <div key={idx} className="h-28 bg-slate-200 rounded-2xl animate-pulse" />
            ))}
          </div>
  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, idx) => (
        <div
          key={idx}
          className="bg-white rounded-3xl border border-slate-200 p-6 h-64 animate-pulse flex flex-col justify-between"
        >
