...
export default function ResourceCard({ resource }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 hover:border-indigo-300 shadow-sm hover:shadow-md transition-all duration-200 p-5 flex flex-col justify-between group">
      <div className="space-y-3">
        ...
        {/* Title & External Link */}
        <h3 className="text-base font-bold text-slate-900 group-hover:text-indigo-600 transition font-sans">
          <a
            href={resource.url}
            target="_blank"
      ...
      <div className="pt-4 mt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
        {resource.author ? (
          <span className="flex items-center gap-1 font-medium text-slate-500 truncate max-w-[180px]">
            <User className="w-3 h-3 text-slate-400 shrink-0" />
            <span className="truncate">{resource.author}</span>
          </span>
        ) : (
          <span className="text-[11px] text-slate-400">External Reference</span>
        )}
