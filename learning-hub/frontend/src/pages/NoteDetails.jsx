      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <nav className="flex flex-wrap items-center gap-1.5 text-xs font-semibold text-slate-500">
          ...
          {topic ? (
            ...
            <Link to="/notes" className="hover:text-indigo-600 transition">
              Notes
            </Link>
          )}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleShare}
            className="inline-flex items-center px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold transition gap-1.5"
          >
            {copiedShare ? (
              <>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {topic && (
            <Link
              to={`/topics/${topic.slug}`}
              className="text-xs font-medium px-3 py-1 rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition"
            >
              Topic: {topic.title}
            </Link>
          )}
          ...
          {!note.published && (
            <span className="text-xs font-bold px-3 py-1 rounded-full bg-amber-500/10 text-amber-600 border border-amber-300">
              Draft / Unpublished
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100 text-xs text-slate-500">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-1.5 bg-amber-50 text-amber-800 font-semibold px-2.5 py-1 rounded-md">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-200">
        {prevNote ? (
          <Link
            ...
            className="w-full sm:w-auto inline-flex items-center px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 transition gap-2"
          >
            <ArrowLeft className="w-4 h-4" />
            <div className="text-left">
              <span className="block text-[10px] uppercase text-slate-400">Previous Note</span>
        ...
        {topic && (
          <Link
            to={`/topics/${topic.slug}`}
            className="inline-flex items-center px-4 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold transition"
          >
        ...
        {nextNote ? (
          <Link
            ...
            className="w-full sm:w-auto inline-flex items-center justify-end px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 transition gap-2 text-right"
          >
            <div>
              <span className="block text-[10px] uppercase text-slate-400">Next Note</span>
