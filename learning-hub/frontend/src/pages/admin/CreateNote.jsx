...
export default function CreateNote() {
  ...
  const { actionLoading, error } = useSelector((state) => state.notes);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.content.trim()) {
      setFormError('Note markdown content is required.');
      return;
    }
    ...
    try {
      const result = await dispatch(createNote(notePayload)).unwrap();
      navigate(`/notes/${result.slug}`);
    } catch (err) {
      setFormError(err || 'Failed to create note');
    }
  };
      ...
      <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 rounded-3xl p-8 text-white shadow-xl flex items-center justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
            Add New Note to Curriculum
          </h1>
          <p className="text-slate-300 text-xs sm:text-sm">
            Draft learning guides with rich markdown, interactive code examples, and clear takeaways.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <label htmlFor="published" className="text-xs font-semibold text-slate-700 cursor-pointer">
            Publish this note immediately (visible to all students)
          </label>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
          ...
          >
            ...
            <span>{actionLoading ? 'Saving Note...' : 'Create Note'}</span>
          </button>
        </div>
      </form>
    </div>
