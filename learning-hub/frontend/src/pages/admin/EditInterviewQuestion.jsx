...
export default function EditInterviewQuestion() {
  ...
  useEffect(() => {
    if (question) {
      setFormData({
        ...
        frequency: question.frequency || 'COMMON',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(
        updateInterviewQuestion({ id, data: payload })
      ).unwrap();
      navigate('/admin/interview-questions');
    } catch (err) {
      setFormError(err || 'Failed to update interview question');
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <p className="text-xs text-slate-500 font-medium">
          Loading interview question details...
        </p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-slate-900 via-rose-950 to-indigo-950 rounded-3xl p-8 text-white shadow-xl flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-slate-300 text-xs sm:text-sm line-clamp-1">
            {formData.question || 'Update interview prompt, solution, or difficulty'}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            onChange={handleChange}
            placeholder="e.g., What is the Temporal Dead Zone (TDZ) in JavaScript?"
            required
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            onChange={handleChange}
            placeholder="Provide a clear, articulate, senior-level explanation..."
            required
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
          ...
          >
            ...
            <span>{actionLoading ? 'Saving...' : 'Update Question'}</span>
          </button>
        </div>
      </form>
    </div>
