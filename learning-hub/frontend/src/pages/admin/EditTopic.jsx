...
export default function EditTopic() {
  ...
  const { currentTopic: topic, detailsLoading, actionLoading, error } = useSelector(
    (state) => state.topics
  );
  ...
  useEffect(() => {
    if (topic) {
      setFormData({
        title: topic.title || '',
        slug: topic.slug || '',
        section: topic.sectionId || topic.section?.id || '',
        summary: topic.summary || '',
        description: topic.description || '',
        duration: topic.duration || '15 mins',
        order: topic.order ?? 1,
        keyPointsInput: Array.isArray(topic.keyPoints) ? topic.keyPoints.join(', ') : '',
        content: topic.content || '',
        published: topic.published !== false,
      });

      if (Array.isArray(topic.codeExamples) && topic.codeExamples.length > 0) {
        setCodeExamples(topic.codeExamples);
      } else {
        setCodeExamples([]);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(updateTopic({ id, data: payload })).unwrap();
      navigate('/admin/topics');
    } catch (err) {
      setFormError(err || 'Failed to update topic');
    }
  };

  if (detailsLoading && !topic) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center text-slate-400">
        ...
        <p className="text-sm font-medium">Loading topic details...</p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 rounded-3xl p-8 text-white shadow-xl">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight font-mono">
            Edit: {formData.title || 'Topic'}
          </h1>
          <p className="text-slate-300 text-xs sm:text-sm">
            Update syntax explanations, code examples, checklist mechanics, and publication status.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            ...
            {activeContentTab === 'write' ? (
              ...
              <div className="rounded-2xl border border-slate-200 overflow-hidden bg-slate-50/50">
                <TopicContentReader content={formData.content || '*No content entered yet.*'} />
              </div>
            )}
          </div>
